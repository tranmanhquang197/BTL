...
@Component({
  selector: 'ns-multi-select-api-autocomplete',
  template: `
...
export class NsMultiSelectSeachApiAutocompleteComponent implements ControlValueAccessor, OnInit, OnChanges {
  ...
  filterItem(value: string) {
    ...
    this.filteredOptions = this.options.filter((item: SelectModel) => {
          ...
          if (translatedDisplayValue) {
            return translatedDisplayValue.toLowerCase().indexOf(value.toLowerCase()) > -1;
          } else {
            return item.displayValue.toLowerCase().indexOf(value.toLowerCase()) > -1;
          }
        },
    );
